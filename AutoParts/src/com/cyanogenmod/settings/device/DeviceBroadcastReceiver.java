...
public class DeviceBroadcastReceiver extends BroadcastReceiver {
    ...
    private void checkKeyPadKeyPressed(Context ctx, Intent intent) {
        ...
        int key = intent.getIntExtra(
                KeyHandler.EXTRA_ASUSPEC_KEY, KeyHandler.ASUSPEC_UNKNOWN);
        int status = intent.getIntExtra(
                KeyHandler.EXTRA_ASUSPEC_STATUS, KeyHandler.ASUSPEC_STATUS_OFF);
        String title = null;
        int icon = com.android.internal.R.drawable.ic_settings_language;

        // Choose the notification from key
        switch (key) {
            case KeyHandler.ASUSPEC_WIFI:
                title = ctx.getString(
                        (status == KeyHandler.ASUSPEC_STATUS_ON) ?
                            R.string.kp_wifi_on :
                            R.string.kp_wifi_off);
                break;

            case KeyHandler.ASUSPEC_BT:
                title = ctx.getString(
                        (status == KeyHandler.ASUSPEC_STATUS_ON) ?
                            R.string.kp_bt_on :
                            R.string.kp_bt_off);
            ...
            case KeyHandler.ASUSPEC_TOUCHPAD:
                title = ctx.getString(
                        (status == KeyHandler.ASUSPEC_STATUS_ON) ?
                            R.string.kp_tp_on :
                            R.string.kp_tp_off);
            ...
            case KeyHandler.ASUSPEC_BRIGHTNESS:
                if (status == KeyHandler.ASUSPEC_STATUS_ON) {
                    title = ctx.getString(R.string.kp_brightness_auto);
                } else {
                    int value = intent.getIntExtra(KeyHandler.EXTRA_ASUSPEC_VALUE, 0);
                    title = ctx.getString(R.string.kp_brightness_manual, value);
                }
                break;

            case KeyHandler.ASUSPEC_VOLUME:
                int value = intent.getIntExtra(KeyHandler.EXTRA_ASUSPEC_VALUE, 0);
                title = ctx.getString(R.string.kp_volume, String.valueOf(value));
                break;

            case KeyHandler.ASUSPEC_VOLUME_MUTE:
                title = ctx.getString(
                        (status == KeyHandler.ASUSPEC_STATUS_ON) ?
            ...
            case KeyHandler.ASUSPEC_MEDIA:
                int res = R.string.kp_media_play_pause;
                if (status == KeyHandler.ASUSPEC_MEDIA_PREVIOUS) {
                    res = R.string.kp_media_previous;
                } else if (status == KeyHandler.ASUSPEC_MEDIA_NEXT) {
                    res = R.string.kp_media_next;
                }
                title = ctx.getString(res);
                break;

            case KeyHandler.ASUSPEC_SCREENSHOT:
                title = ctx.getString(R.string.kp_screenshot);
                break;

            case KeyHandler.ASUSPEC_EXPLORER:
                title = ctx.getString(R.string.kp_explorer);
                break;

            case KeyHandler.ASUSPEC_SETTINGS:
                title = ctx.getString(R.string.kp_settings);
                break;

            case KeyHandler.ASUSPEC_CAPS_LOCK:
                title = ctx.getString(
                        (status == KeyHandler.ASUSPEC_STATUS_ON) ?
