...
public final class KeyHandler implements DeviceKeyHandler {
    private static final String TAG = "AsuspecKeyHandler";
    ...
    // Private asuspec keys values (for notification purpose)
    public static final int ASUSPEC_UNKNOWN          = -1;
    public static final int ASUSPEC_WIFI             =  1;
    public static final int ASUSPEC_BT               =  2;
    public static final int ASUSPEC_TOUCHPAD         =  3;
    public static final int ASUSPEC_BRIGHTNESS       =  4;
    public static final int ASUSPEC_VOLUME           =  5;
    public static final int ASUSPEC_VOLUME_MUTE      =  6;
    public static final int ASUSPEC_MEDIA            =  7;
    public static final int ASUSPEC_SCREENSHOT       =  8;
    public static final int ASUSPEC_EXPLORER         =  9;
    public static final int ASUSPEC_SETTINGS         = 10;
    public static final int ASUSPEC_CAPS_LOCK        = 11;

    public static final int ASUSPEC_STATUS_OFF       =  0;
    public static final int ASUSPEC_STATUS_ON        =  1;

    public static final int ASUSPEC_MEDIA_PLAY_PAUSE =  0;
    public static final int ASUSPEC_MEDIA_PREVIOUS   =  1;
    public static final int ASUSPEC_MEDIA_NEXT       =  2;

    // Use specific scan codes from device instead of aosp keycodes
    private static final int SCANCODE_F1     = 59;
    private static final int SCANCODE_F2     = 467;
    private static final int SCANCODE_F3     = 468;
    private static final int SCANCODE_F4     = 469;
    ...
    public boolean handleKeyEvent(KeyEvent event) {
        ...
        // Asuspec should only handle key events when the device is docked.
        // The device should be a full keyboard like the Transformers one (otherwise we
        // are in present of a gpio interruption)
        // TODO The check for ALPHA need to be verified (on TF701T ALPHA is mapped to gpio
        ...
        switch (event.getScanCode()) {
            ...
            case SCANCODE_VOLUME_MUTE:
                ...
                // treat this event as a volume mute toggle action. the asuspec KeyHandler
    ...
    private void toggleWifi() {
        ...
        if (state == WifiManager.WIFI_STATE_ENABLED
            ...
            notifyKey(ASUSPEC_WIFI, ASUSPEC_STATUS_OFF);

        } else if (state == WifiManager.WIFI_STATE_DISABLED
                && apState == WifiManager.WIFI_AP_STATE_DISABLED) {
            mWifiManager.setWifiEnabled(true);
            notifyKey(ASUSPEC_WIFI, ASUSPEC_STATUS_ON);
        }
    }
    ...
    private void toggleBluetooth() {
        ...
        if (state == BluetoothAdapter.STATE_OFF) {
            ...
            notifyKey(ASUSPEC_BT, ASUSPEC_STATUS_ON);
        }
        if (state == BluetoothAdapter.STATE_ON) {
            mBluetoothAdapter.disable();
            notifyKey(ASUSPEC_BT, ASUSPEC_STATUS_OFF);
        }
    }
    ...
    private void toggleTouchpad() {
        ...
        notifyKey(ASUSPEC_TOUCHPAD, mTouchpadEnabled ? ASUSPEC_STATUS_ON : ASUSPEC_STATUS_OFF);
    }
    ...
    private void brightnessDown() {
        ...
        setBrightness(value);
        notifyKey(ASUSPEC_BRIGHTNESS, ASUSPEC_STATUS_OFF, value);
    }
    ...
    private void toggleAutoBrightness() {
        ...
        int status = currentValue == Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL ?
                                    ASUSPEC_STATUS_ON :
                                    ASUSPEC_STATUS_OFF;
        notifyKey(ASUSPEC_BRIGHTNESS, status, value);
    }
    ...
    private void launchExplorer() {
        notifyKey(ASUSPEC_EXPLORER, ASUSPEC_STATUS_OFF);
    }

    private void launchSettings() {
        try {
            mContext.startActivity(mSettingsIntent);
            notifyKey(ASUSPEC_SETTINGS, ASUSPEC_STATUS_OFF);
    ...
    private void toggleAudioMute() {
        ...
        if (mAudioManager.getMode() == AudioManager.MODE_NORMAL) {
            ...
            int status = newValue == AudioManager.RINGER_MODE_NORMAL ?
                    ASUSPEC_STATUS_OFF :
                    ASUSPEC_STATUS_ON;
            notifyKey(ASUSPEC_VOLUME_MUTE, status);
        }
    }
    ...
    private void volumeDown() {
        ...
        int status = mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_NORMAL ?
                    ASUSPEC_STATUS_ON :
                    ASUSPEC_STATUS_OFF;
        notifyKey(ASUSPEC_VOLUME, status, value);
    }
    ...
    private void mediaPlayPause() {
        notifyKey(ASUSPEC_MEDIA, ASUSPEC_MEDIA_PLAY_PAUSE);
    }

    private void mediaPrevious() {
        notifyKey(ASUSPEC_MEDIA, ASUSPEC_MEDIA_PREVIOUS);
    }

    private void mediaNext() {
        notifyKey(ASUSPEC_MEDIA, ASUSPEC_MEDIA_NEXT);
    }
    ...
    private void capsLock(KeyEvent event) {
        int status = event.getMetaState() == KeyEvent.META_CAPS_LOCK_ON ?
                ASUSPEC_STATUS_ON :
                ASUSPEC_STATUS_OFF;
        notifyKey(ASUSPEC_CAPS_LOCK, status);
    }
    ...
    private void takeScreenshot() {
        synchronized (mScreenshotLock) {
            ...
            notifyKey(ASUSPEC_SCREENSHOT, ASUSPEC_STATUS_OFF);
    ...
    private void notifyKey(int asuspeckey, int status, int value) {
        Intent intent = new Intent();
        intent.setAction(ACTION_DOCK_KEYPAD_KEY_PRESSED);
        intent.putExtra(EXTRA_ASUSPEC_KEY, asuspeckey);
        intent.putExtra(EXTRA_ASUSPEC_STATUS, status);
        intent.putExtra(EXTRA_ASUSPEC_VALUE, value);
        mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT, PERMISSION_KEYPAD_RECEIVER);
    }

    private void notifyKey(int asuspeckey, int status) {
        notifyKey(asuspeckey, status, 0);
    }
